...
if /i "!ADVANCED_CONFIG!"=="EXIT" exit /b
echo.
...
echo [9] Build EXPERIMENTAL MP rend2         - !BuildMPRend2!
echo [A] Build with Discord Rich Presence    - !BuildDiscordRichPresence!
echo [B] Build unit tests (requires Boost)   - !BuildTests!
echo [C] Set custom install path             - [!INSTALL_PATH!]
echo [D] Add custom CMake options            - [!AdditionalCMakeOptions!]
echo [E] Done
echo [F] Quit
echo.
set /p CONFIG_CHOICE="Select an option to toggle or configure (1-9, A-F): "
...
IF /I "!CONFIG_CHOICE!"=="C" (
    echo Enter custom install path or leave empty to keep current [!INSTALL_PATH!]:
    set /p "NEW_INSTALL_PATH=Custom install path: "
    if not "!NEW_INSTALL_PATH!"=="" SET "INSTALL_PATH=!NEW_INSTALL_PATH!"
...
:CONFIG_DONE
SET "CMAKE_OPTIONS=-DBuildPortableVersion=!BuildPortableVersion!"
...
:: Prompt before clearing cache if it already exists
IF EXIST "!BUILD_FOLDER!\CMakeCache.txt" (
    echo Existing CMake cache found in "!BUILD_FOLDER!".
    set /p CLEAR_CACHE="Clear cache and reconfigure from scratch? (Y/N, default N): "
    IF /I "!CLEAR_CACHE!"=="Y" (
        DEL /F /Q "!BUILD_FOLDER!\CMakeCache.txt"
        RMDIR /S /Q "!BUILD_FOLDER!\CMakeFiles"
        echo Cache cleared.
    ) ELSE (
...
:: Subroutine to toggle ON/OFF configuration options
:TOGGLE_OPTION
SET "optionName=%1"
IF "!%optionName%!"=="ON" (
    SET "!optionName!=OFF"
