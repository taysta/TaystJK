...
void DF_StrafeHelper() {
	...
	float activeMin, rearActiveMin, activeOpt, rearActiveOpt, activeMax, rearActiveMax, activeMaxCos, rearActiveMaxCos;
	int i;
	...
	if (!(state.cmd.forwardmove == 0 && state.cmd.rightmove == 0)) //only do this if keys are pressed
	{
		//active min angle
...
void DF_SetCGAZ() {
	...
	state.cgaz.vf = (state.onGround && state.cgaz.wasOnGround && !state.onSlick) ? (state.cgaz.s * (1 - state.physics.friction * state.cgaz.frametime)) : state.cgaz.v;
...
void DF_DrawStrafeLine(dfsline line) {
	...
	if (cg_strafeHelper.integer & SHELPER_CGAZ) { //draw the cgaz style strafehelper
		if (cg_strafeHelperCutoff.integer > 256) {
			DF_DrawLine(line.x, (0.5f * SCREEN_HEIGHT) - 5.0f, line.x, (0.5f * SCREEN_HEIGHT) + 5.0f, state.strafeHelper.lineWidth, line.color, 0.75f, 0); //maximum cutoff
		}
		else {
			DF_DrawLine(line.x, (0.5f * SCREEN_HEIGHT) - 20.0f + cg_strafeHelperCutoff.value / 16.0f, line.x, (0.5f * SCREEN_HEIGHT) + 20.0f - cg_strafeHelperCutoff.value / 16.0f, state.strafeHelper.lineWidth, line.color, 0.75f, 0); //default/custom cutoff
		}
	}
...
*/
 void DF_DrawSpeedometer(void) {
	...
	{
		Com_sprintf(speedStr2, sizeof(speedStr2), "   %.1f", currentSpeed);
	...
	{
		Com_sprintf(speedStr3, sizeof(speedStr3), "   %.1f", currentSpeed);
...
void DF_UpdateSnapHudSettings(float speed, int fps, qboolean pro) {
	...
	for (step = (int)(floor(speed / (float)state.snappinghud.fps + 0.5) - 0.5); step > 0 && state.snappinghud.count < SNAPHUD_MAXZONES - 2; step--) {
		state.snappinghud.zones[state.snappinghud.count] = RAD2DEG(acosf(step / (speed / (float)state.snappinghud.fps)));
		state.snappinghud.count++;
		state.snappinghud.zones[state.snappinghud.count] = RAD2DEG(asinf(step / (speed / (float)state.snappinghud.fps)));
		state.snappinghud.count++;
	}
...
void DF_DrawSnapHud(void)
{
	...
	speed = cg_snapHudSpeed.integer ? (float)cg_snapHudSpeed.integer : state.cgaz.s;
	if (state.moveStyle == MV_OCPM) {
		fps = 125;
	}
	else {
